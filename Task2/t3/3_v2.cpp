...
#pragma omp parallel shared(x, x_new, b, iter) 
    {
        bool converged = false;

        while (!converged && iter < max_iter) {
            double sum_all_x = 0.0;
            double norm_res_local = 0.0;
...
#pragma omp for reduction(+:norm_res_local)
            for (int i = 0; i < N; ++i) {
                double Ax = sum_all_x + x[i];
                double res = Ax - b[i];
                norm_res_local += res * res;
                x_new[i] = x[i] - tau * res;
            }

#pragma omp single
            {
                double norm_res = std::sqrt(norm_res_local);
