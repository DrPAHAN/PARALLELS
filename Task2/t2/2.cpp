...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cout << "Usage: ./integrate num_threads\n";
        return 1;
    }

    int threads = std::stoi(argv[1]);
    long long nsteps = 40'000'000;

    std::cout << "=== Numerical integration (nsteps = " << nsteps << ") ===\n";

    // Можно замерить оба варианта
    double t_atomic = integrate_omp_atomic(nsteps, threads);
    double t_local  = integrate_omp_local(nsteps, threads);
