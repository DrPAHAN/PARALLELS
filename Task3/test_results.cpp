...
int main() {
    bool all_pass = true;
    // Явные лямбды решают проблему перегрузки std::sin/std::sqrt
    all_pass &= verify_file("sin_results.csv", [](double x){ return std::sin(x); });
    all_pass &= verify_file("sqrt_results.csv", [](double x){ return std::sqrt(x); });
